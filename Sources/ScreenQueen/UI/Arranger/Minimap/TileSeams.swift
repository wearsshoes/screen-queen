...
extension Minimap {
    ...
    func miniBarEdges(_ bars: [SeamBar], t: Transform,
                      seamColor: [DisplayGraph.SeamKey: NSColor]) -> [Stage.SeamEdgeGlow] {
        let thickness: CGFloat = 5, gap: CGFloat = 2
        // Trim the ends clear of the rounded corners.
        func barLen(_ inches: CGFloat) -> CGFloat { stage.trimmedBarLength(inches * t.scale, cap: 8) }
        var edges: [Stage.SeamEdgeGlow] = []
        for bar in bars {
            ...
            if bar.isVertical {
                ...
                edges.append(Stage.SeamEdgeGlow(rect: NSRect(x: cA.x - gap - thickness, y: cA.y - lenA / 2, width: thickness, height: lenA), inward: .minX, color: color))
                edges.append(Stage.SeamEdgeGlow(rect: NSRect(x: cB.x + gap, y: cB.y - lenB / 2, width: thickness, height: lenB), inward: .maxX, color: color))
            } else {
                ...
                edges.append(Stage.SeamEdgeGlow(rect: NSRect(x: cA.x - lenA / 2, y: cA.y - gap - thickness, width: lenA, height: thickness), inward: .minY, color: color))
                edges.append(Stage.SeamEdgeGlow(rect: NSRect(x: cB.x - lenB / 2, y: cB.y + gap, width: lenB, height: thickness), inward: .maxY, color: color))
            }
        }
        return edges
    }
