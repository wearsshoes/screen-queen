#!/bin/bash
# Fast dev loop: rebuild (debug) and relaunch Screen Queen, killing the old instance.
#
...
run() {
	...
	pkill -f "/debug/ScreenQueen$" 2>/dev/null || true
	for _ in $(seq 1 20); do pgrep -f "/debug/ScreenQueen$" >/dev/null || break; sleep 0.1; done
	# Launch the binary directly (not the .app) so stdout/logs stream to this terminal.
	.build/debug/ScreenQueen &
